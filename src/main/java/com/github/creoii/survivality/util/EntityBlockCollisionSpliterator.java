...
public class EntityBlockCollisionSpliterator extends AbstractIterator<VoxelShape> {
    ...
    private final RegistryEntryList<Block> blacklist;

    public EntityBlockCollisionSpliterator(CollisionView world, Entity entity, Box box, RegistryEntryList<Block> blacklist) {
        ...
        while (true) {
            if (blockIterator.step()) {
                ...
                for (int b = 0; b < blacklist.size(); ++b) {
                    if (state.isOf(blacklist.get(b).value())) {
                        skip = true;
                        break;
                    }
                }
